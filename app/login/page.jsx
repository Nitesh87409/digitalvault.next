...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    setIsInAppBrowser(/FBAN|FBAV|Instagram|Line|Twitter|LinkedInApp/i.test(ua));
  }, []);
  ...
  // Load social SDKs
  useEffect(() => {
    if (settings.google_login_enabled && !isInAppBrowser && window.google) {
      initGoogle();
    }
  ...
  }, [settings.google_login_enabled, settings.apple_login_enabled, isInAppBrowser]);

  const openLoginInExternalBrowser = () => {
    const ua = navigator.userAgent || '';
    if (/Android/i.test(ua)) {
      window.location.href = 'intent://www.downloadkart.com/login#Intent;scheme=https;package=com.android.chrome;end';
      return;
    }
    setError('Google Sign-In is blocked inside this app browser. Please tap the menu and choose Open in external browser, or use Email OTP/password.');
  };
    ...
    <div className="font-sans bg-[#0a0a0f] min-h-screen flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center py-4 md:py-8 px-4 relative z-[1]">
        <div className="bg-[#12121a] border border-[#f5c842]/15 rounded-[16px] md:rounded-[20px] p-5 sm:p-7 md:p-9 w-full max-w-[420px] shadow-[0_20px_40px_rgba(0,0,0,0.4)]">
                ...
                <>
                  ...
                  <div className="flex flex-col gap-3">
                    {settings.google_login_enabled && (
                      isInAppBrowser ? (
                        <div className="rounded-xl border border-[#f5c842]/25 bg-[#f5c842]/10 p-3 text-center">
                          <p className="mb-3 text-[0.82rem] leading-5 text-[#f5c842]">
                            Google Sign-In is blocked inside Facebook/Instagram browser. Open in Chrome/Safari or use Email OTP/password.
                          </p>
                          <button
                            type="button"
                            onClick={openLoginInExternalBrowser}
                            className="w-full rounded-xl border-none bg-white px-4 py-3 font-sans text-[0.95rem] font-semibold text-black"
                          >
          ...
          {settings.google_login_enabled && !isInAppBrowser && (
            <Script
              src="https://accounts.google.com/gsi/client"
              onLoad={initGoogle}
              onError={() => setError('Google Sign-In failed to load. Please use Email OTP/password or open this page in an external browser.')}
