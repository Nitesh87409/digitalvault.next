...
export async function POST(request) {
  try {
    ...
    const ipLimit = await consumePersistentRateLimit(buildRateLimitKey(request, "otp-send-email-ip"), SEND_EMAIL_LIMIT);
    if (!ipLimit.allowed) {
      return NextResponse.json({ flag: 0, message: "Too many requests. Please try again later." }, { status: 429 });
    }

    const emailLimit = await consumePersistentRateLimit(buildRateLimitKey(request, "otp-send-email", email), SEND_EMAIL_LIMIT);
