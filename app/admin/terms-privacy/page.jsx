...
export default function AdminTermsPrivacyPage() {
  ...
  const loadRichEditor = (termsContent) => {
    setTimeout(() => {
      if (termsRef.current) {
        termsRef.current.innerHTML = termsContent || '';
      }
    }, 50);
  };
  ...
  async function fetchSettings() {
    try {
      ...
      if (data.flag && data.settings) {
        const termsContent = data.settings.terms_privacy_content ?? '';
        setContent(termsContent);
        loadRichEditor(termsContent);
      }
    } catch (e) {
      console.error(e);
  ...
  async function handleSave() {
    setSaving(true);
    setMessage('');
    const newContent = termsRef.current?.innerHTML || '';

    try {
      const res = await fetch('/api/settings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ terms_privacy_content: newContent })
      });
      const data = await res.json();
      if (data.flag) {
        setContent(newContent);
        setMessage('Terms & Privacy policy saved successfully! 📜');
      } else {
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 shrink-0">
        <div>
          <h1 className="font-syne text-2xl md:text-3xl font-bold text-white tracking-tight">📜 Terms & Privacy Policy</h1>
          <p className="text-gray-500 text-sm mt-1">Manage and edit your website terms, service conditions, and privacy policies.</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-[#0e0e18] rounded-2xl p-5 sm:p-8 border border-white/5 shadow-xl animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <div className="flex items-center gap-4 pt-4 border-t border-white/5">
              ...
              >
                {saving ? 'Saving...' : '💾 Save Terms & Privacy'}
              </button>
              {message && (
