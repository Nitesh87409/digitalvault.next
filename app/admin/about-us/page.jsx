...
export default function AdminAboutUsPage() {
  ...
  const loadRichEditor = (aboutUsContent) => {
    setTimeout(() => {
      if (aboutUsRef.current) {
        aboutUsRef.current.innerHTML = aboutUsContent || '';
      }
    }, 50);
  };
  ...
  async function fetchSettings() {
    try {
      ...
      if (data.flag && data.settings) {
        const aboutUsContent = data.settings.about_us_content ?? '';
        setContent(aboutUsContent);
        loadRichEditor(aboutUsContent);
      }
    } catch (e) {
      console.error(e);
  ...
  async function handleSave() {
    setSaving(true);
    setMessage('');
    const newContent = aboutUsRef.current?.innerHTML || '';

    try {
      const res = await fetch('/api/settings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ about_us_content: newContent })
      });
      const data = await res.json();
      if (data.flag) {
        setContent(newContent);
        setMessage('About Us saved successfully! ℹ️');
      } else {
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 shrink-0">
        <div>
          <h1 className="font-syne text-2xl md:text-3xl font-bold text-white tracking-tight">ℹ️ About Us</h1>
          <p className="text-gray-500 text-sm mt-1">Manage and edit the dynamic About Us page content, mission statement, and details.</p>
        </div>
